var CMSGridPager = function () {
   ...
   this.createPager = function (cmsGrid) {
      ...
      var colspan = cmsGrid.fields.length;
      var $tableRows = cmsGrid.body.children();

      var $tableFooter = self.tableFooter = $("<tfoot>");
      $tableFooter.addClass("cms-grid-footer");

      var $tableFooterRow = $("<tr>");
      var $pagerWrapper = $("<td colspan='" + colspan + "'>");

      $pagerWrapper.addClass("cms-grid-pager-wrapper");
   ...
   this.createPagerContent = function () {
      ...
      $pager.append("<li class='cms-grid-pager-label'>Page:</li>");
      $pager.append("<li><input type='text' class='cms-grid-pager-input' value='1'/></li>");
      $pager.append("<li class='cms-grid-pager-label' id='changePageNum'>of " + self.numberOfPages + "</li>");
   ...
   this.setEventHandlersForPager = function () {
      ...
      $pager.find("li .cms-grid-pager-input").keypress(function (e) {
         if (e.keyCode === 13) {
            var page = this.value;
      ...
      $pager.find(".cms-grid-pager-picker").change(function () {
         var selected = parseInt($(".cms-grid-pager-picker option:selected").text());
   ...
   this.jumpToPage = function (page) {      
      ...
      $("li .cms-grid-pager-input").val(++page);
      ...
      $(self.gridPager).find(".cms-grid-pager-status").text("View " + self.statusFrom + " - " + self.statusTo + " of " + self.grid.body.children().size());
   ...
   this.createPagesPicker = function () {
      ...
      $picker.addClass("cms-grid-pager-picker");
   ...
   this.createStatusLabel = function () {
      ...
      $statusLabel.addClass("cms-grid-pager-status");
