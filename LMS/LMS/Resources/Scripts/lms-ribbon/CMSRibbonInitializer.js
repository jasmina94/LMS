...
/// <reference path="~/Resources/Scripts/cms-ribbon/CMSRibbonFactory.js" />
/// <reference path="~/Resources/Scripts/ajax/AjaxHttpSender.js" />
/// <reference path="~/Resources/Scripts/cms-grid/CMSGrid.js" />
/// <reference path="~/Resources/Scripts/security/CMSSecurityProvider.js" />

var cmsRibbonInitializer = function ($) {
   var ribbonEnum = new CMSRibbonEnum();
   var ribbonFactory = cmsRibbonFactory();
   ...
   function changePanel(htmlPanel) {
      ...
      if ($content.length == 0) {
         ...
         if (newContent && nextState != currentState) {
            ...
            var $cmsGrid = $(".cms-grid-container");
            var grid = $cmsGrid.data("CMSGrid");
